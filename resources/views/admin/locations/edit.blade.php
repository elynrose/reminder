...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.locations.update", [$location->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="gname">{{ trans('cruds.location.fields.gname') }}</label>
                <input class="form-control {{ $errors->has('gname') ? 'is-invalid' : '' }}" type="text" name="gname" id="gname" value="{{ old('gname', $location->gname) }}">
                @if($errors->has('gname'))
                    <div class="invalid-feedback">
                        {{ $errors->first('gname') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.location.fields.gname_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="lat">{{ trans('cruds.location.fields.lat') }}</label>
                <input class="form-control {{ $errors->has('lat') ? 'is-invalid' : '' }}" type="number" name="lat" id="lat" value="{{ old('lat', $location->lat) }}" step="0.0000001">
                @if($errors->has('lat'))
                    <div class="invalid-feedback">
                        {{ $errors->first('lat') }}
                    </div>
            ...
            <div class="form-group">
                <label for="lng">{{ trans('cruds.location.fields.lng') }}</label>
                <input class="form-control {{ $errors->has('lng') ? 'is-invalid' : '' }}" type="number" name="lng" id="lng" value="{{ old('lng', $location->lng) }}" step="0.0000001">
                @if($errors->has('lng'))
                    <div class="invalid-feedback">
                        {{ $errors->first('lng') }}
                    </div>
            ...
            <div class="form-group">
                <label class="required" for="user_id">{{ trans('cruds.location.fields.user') }}</label>
